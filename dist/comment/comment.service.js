...
let CommentService = class CommentService {
    ...
    async createComment(dto, user) {
        const { blogId } = dto;
        const blog = await this.dataSource.getRepository(blog_entity_1.Blog).createQueryBuilder('Blog').select("blog").from(blog_entity_1.Blog, 'blog').where('blog.id = :blogId', { blogId }).getOne();
